...
API_VERSION = 3
COMMANDS = ('groups feeds items unread_item_ids saved_item_ids mark '
            'unread_recently_read favicons links'.split())
...
class FeverApp(WSGIApp, FeedController, UserController):
    ...
    @POST(r'^/fever/?$')
    def endpoint(self):
        logger.debug(u'client from %s requested: %s' % (
            self.request.remote_addr, self.request.params))
    ...
    def unread_item_ids_command(self, result):
        q = self.get_unread_entries(Entry.id).objects()
        ids = [r.id for r in q]
        result.unread_item_ids = ','.join(map(str, ids))

    def saved_item_ids_command(self, result):
        q = self.get_saved_entries(Entry.id).objects()
        ids = [r.id for r in q]
    ...
    def unread_recently_read_command(self, result):
        ...
        q = Read.delete().where((Read.user == self.user
                                 ) & (Read.read_on > since))
        count = q.execute()
    ...
    def mark_command(self, result):
        ...
        try:
            mark, status, object_id = self.request.POST['mark'], \
                self.request.POST['as'], int(self.request.POST['id'])
        ...
        if mark == 'item':
            ...
            if status == 'read':
                try:
                    ...
                    logger.debug(
                        u'entry %d already marked as read, ignored' %
                        object_id)
                    return
            ...
            #  the Fever API, but Reeder app asks for it
                ...
                count = Read.delete().where(
                    (Read.user == self.user) & (Read.entry == entry)).execute()
                if not count:
                    logger.debug(
                        u'entry %d never marked as read, ignored' % object_id)
                    return
            elif status == 'saved':
                try:
                    Saved.create(user=self.user, entry=entry)
                except IntegrityError:
                    logger.debug(u'entry %d already marked as saved, ignored'
                                 % object_id)
                    return
            elif status == 'unsaved':
            ...
            try:
                ...
                logger.debug(
                    u'missing or invalid parameter (%s), ignored' % ex)
                return
            ...
            if object_id == 0:
                ...
                try:
                    ...
                    logger.debug(
                        u'could not find group %d, ignored' % object_id)
                    return
            ...
            logger.debug(
                u'malformed mark command (mark %s (%s) as %s ), ignored' % (
